...
//|             Extracted from SwingMasterPro -- Panel only           |
//|                                                                  |
//| v2.0 -- Bias tuning: TrendGapThreshold, UseDoubleBarSlope,      |
//|         D1_Weight, BiasStrongThreshold, BiasRSIFilter.           |
//|         Optimized: all indicator calls cached once per tick.      |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| CACHE -- computed once per tick, reused everywhere                |
//| Eliminates redundant indicator calls per tick.                    |
//+------------------------------------------------------------------+

// TF sign trend cache: g_tfSign[pairIndex][tfIdx]
// tfIdx: 0=D1, 1=H4, 2=H1, 3=M30, 4=M15
int      g_tfSign[64][5];
datetime g_cacheTime = 0;

// Currency bias label cache (derived from g_tfSign)
string g_biasH1[7];    // GetBiasLabel results  (D1+H4+H1 authority)
string g_biasM15[7];   // GetBiasLabelLow results (H1+M30+M15 authority)

// Per-pair indicator cache for tradeable display section
struct PairCache {
   double h1_ema20;
   double h1_atr;
   double h1_close;
...
// Currency index mapping
int CurIdx(string cur) {
   for(int i = 0; i < 7; i++)
...
//+------------------------------------------------------------------+
//| CURRENCY STRENGTH / BIAS ENGINE (cached)                         |
//| All functions read from g_tfSign[] cache populated once per tick. |
//+------------------------------------------------------------------+

// Raw TF sign computation -- called ONLY by UpdateAllCaches
// v2.0: uses TrendGapThreshold + UseDoubleBarSlope tuning
int ComputeTFSignTrend(string sym, int tf) {
...
// Cached currency score -- reads from g_tfSign[] (no indicator calls)
int CurrencyScoreInt(string cur, int tf) {
   int tfi = TFIdx(tf);
   if(tfi < 0) return 0;
   int bullish = 0, bearish = 0;
   for(int i = 0; i < PairsCount; i++) {
      string base = StringSubstr(Pairs[i], 0, 3);
      string quot = StringSubstr(Pairs[i], 3, 3);
      if(base != cur && quot != cur) continue;
      int sign = g_tfSign[i][tfi];
      if(sign == 0) continue;
...
// Cached currency score detail -- reads from g_tfSign[] (no indicator calls)
...
// v2.0: D1_Weight + BiasStrongThreshold + BiasRSIFilter tuning
...
//+------------------------------------------------------------------+
//| MASTER CACHE UPDATE -- call once per tick before anything else    |
//+------------------------------------------------------------------+
void UpdateAllCaches() {
   datetime now = TimeCurrent();
   if(g_cacheTime == now) return;   // already updated this tick
   g_cacheTime = now;

   int tfs[5] = {PERIOD_D1, PERIOD_H4, PERIOD_H1, PERIOD_M30, PERIOD_M15};

   // Pass 1: Compute TF sign trends + pair indicators for ALL pairs
   for(int i = 0; i < PairsCount; i++) {
      string sym = Pairs[i];

      // TF sign trends (5 TFs x 4 indicator calls each = 20 calls per pair)
      for(int t = 0; t < 5; t++)
         g_tfSign[i][t] = ComputeTFSignTrend(sym, tfs[t]);

      // H1 + M15 indicator cache for tradeable display section
      g_pc[i].h1_ema20  = iMA(sym, PERIOD_H1, 20, 0, MODE_EMA, PRICE_CLOSE, 1);
      g_pc[i].h1_atr    = iATR(sym, PERIOD_H1, ATRPeriod, 1);
      g_pc[i].h1_close  = iClose(sym, PERIOD_H1, 1);
      g_pc[i].h1_rsi1   = iRSI(sym, PERIOD_H1, RSI_Period, PRICE_CLOSE, 1);
      g_pc[i].h1_rsi2   = iRSI(sym, PERIOD_H1, RSI_Period, PRICE_CLOSE, 2);
      g_pc[i].m15_ema20 = iMA(sym, PERIOD_M15, 20, 0, MODE_EMA, PRICE_CLOSE, 1);
      g_pc[i].m15_atr   = iATR(sym, PERIOD_M15, ATRPeriod, 1);
      g_pc[i].m15_close = iClose(sym, PERIOD_M15, 1);
      g_pc[i].m15_rsi1  = iRSI(sym, PERIOD_M15, RSI_Period, PRICE_CLOSE, 1);
      g_pc[i].m15_rsi2  = iRSI(sym, PERIOD_M15, RSI_Period, PRICE_CLOSE, 2);
   }

   // Pass 2: Compute currency bias labels (reads from g_tfSign -- no indicator calls)
   for(int c = 0; c < 7; c++) {
      g_biasH1[c]  = GetBiasLabel(Currencies[c]);
      g_biasM15[c] = GetBiasLabelLow(Currencies[c]);
   }
}
...
void CreateChartPanel() {
   ...
   CreateLabel("PanelTitle", "Bias Panel Display v2.0", 20, 38, TextColor, 10, "Arial Black");
   ChartRedraw();
}
...
void UpdateChartPanel() {
   ...
   for(int bi = 0; bi < 7; bi++) {
      ...
      string bias  = g_biasH1[bi];  // cached -- no indicator calls
      ...
      CurrencyScoreDetail(cur, PERIOD_D1, d1b, d1br, d1r);   // reads g_tfSign -- no indicator calls
   ...
   for(int mi = 0; mi < 7; mi++) {
      ...
      string bias = g_biasM15[mi];  // cached -- no indicator calls
      string sh   = (bias == "STRONG") ? "STR" : (bias == "WEAK") ? "WK" : (bias == "INV") ? "INV" : "NEU";
      color  bc   = (bias == "STRONG") ? clrLime : (bias == "WEAK") ? clrRed : (bias == "INV") ? clrOrange : clrGray;
      int mh1b, mh1br, mh1r, mm30b, mm30br, mm30r, mm15b, mm15br, mm15r;
      CurrencyScoreDetail(cur, PERIOD_H1,  mh1b,  mh1br,  mh1r);   // reads g_tfSign -- no indicator calls
   ...
   // -- Section 3: Tradeable Now (uses cached bias + cached indicators) --
   ...
   for(int pi = 0; pi < PairsCount; pi++) {
      ...
      int bIdx = CurIdx(pbase), qIdx = CurIdx(pquote);
      if(bIdx < 0 || qIdx < 0) continue;

      // Read cached bias labels -- no GetBiasLabel/GetBiasLabelLow calls
      int dirH1  = GetDirectionFromBiasLabels(g_biasH1[bIdx],  g_biasH1[qIdx]);
      int dirM15 = GetDirectionFromBiasLabels(g_biasM15[bIdx], g_biasM15[qIdx]);
      ...
      // Read from PairCache -- no iMA/iATR/iRSI/iClose calls
      double tEMA20  = g_pc[pi].h1_ema20;
      double tATR    = g_pc[pi].h1_atr;
      double tClose  = g_pc[pi].h1_close;
      double tRSI1   = g_pc[pi].h1_rsi1;
      double tRSI2   = g_pc[pi].h1_rsi2;
      ...
      double tEMA20m  = g_pc[pi].m15_ema20;
      double tATRm    = g_pc[pi].m15_atr;
      double tClosem  = g_pc[pi].m15_close;
      double tRSI1m   = g_pc[pi].m15_rsi1;
      double tRSI2m   = g_pc[pi].m15_rsi2;
...
int OnInit() {
   BuildPairsUniverse();
   CreateChartPanel();
   UpdateAllCaches();   // populate caches before first panel draw
   UpdateChartPanel();
...
void OnTick() {
   ...
   if(uiSec < 1) uiSec = 1;
   if(TimeCurrent() - g_lastUIRefresh < uiSec) return;
   g_lastUIRefresh = TimeCurrent();
   UpdateAllCaches();   // refresh all indicator data once per tick
   UpdateChartPanel();  // draw panel from cached data (zero indicator calls)
}
